...
class LiveDataSource(context: Context, baseDataSource: DataSource?) :
    DataSource{
    ...
    class Factory @JvmOverloads constructor(
        ...
        override fun createDataSource(): LiveDataSource {
            val dataSource = LiveDataSource(context, baseDataSourceFactory.createDataSource())
         ...
         * @param baseDataSourceFactory The [DataSource.Factory] to be used to create base [     ] for [LiveDataSource] instances. The base [     ] is normally an [HttpDataSource], and is responsible for fetching data
    ...
    override fun open(dataSpec: DataSpec): Long {
        ...
        while (retryCount < Integer.MAX_VALUE) { // 최대 재시도 횟수를 지정할 수 있습니다.
            try {
                ...
                // 예외 발생 시 여기에 처리 로직을 추가합니다.
                retryCount++
            }
        }

        // 여기에 최대 재시도 횟수를 초과한 경우의 예외 처리 로직을 추가할 수 있습니다.
        throw IOException("Failed to open data source after $3 retries")
    }

    @Throws(IOException::class)
    override fun read(buffer: ByteArray, offset: Int, length: Int): Int {
        val readSource = Assertions.checkNotNull(dataSource).read(buffer, offset, length)

        if (isRecording == true){
            if (readSource>0 && mFileStream != null){
                mFileStream?.write(buffer, offset, readSource)
            }
        }
        return readSource
    }
    ...
    companion object {
        ...
        var mFileStream : FileOutputStream? = null


        fun setRecording(record:Boolean? = true, file: File? = null){
            isRecording = record != false
            if (record == true){
                try {
